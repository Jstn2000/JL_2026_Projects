...
class SelectTab(ttk.Frame):
    def __init__(self, parent, state_obj, on_image_chosen, keybindings=None):
    ...
    def _build_ui(self):
        ...
        # Mouse wheel over the gallery scrolls the view. Hovering also grabs
        # keyboard focus so the arrow-key / Enter navigation below works
        # without having to click into the gallery first.
    ...
    def _on_canvas_enter(self, event):
        self._bind_wheel(event)
        self.canvas.focus_set()

    def focus_default(self):
        """Grab keyboard focus for this tab's canvas - called by app.py
        whenever the notebook switches to this tab, so arrow-key / Enter
        navigation works immediately without needing a mouse hover first."""
        self.canvas.focus_set()

    # ------------------------------------------------------------------
    # Customizable keyboard shortcuts: arrow keys move the highlighted
    # image without opening it, Enter confirms/opens the highlighted one.
    # Bound directly to the gallery canvas (like the paint canvas's WASD
    # panning) so they don't steal these keys from other tabs/widgets.
    # ------------------------------------------------------------------
    def apply_keybindings(self):
        ...
        bindings = {
            "select_move_up": lambda e: self._move_cursor(0, -1),
            "select_move_down": lambda e: self._move_cursor(0, 1),
            "select_move_left": lambda e: self._move_cursor(-1, 0),
            "select_move_right": lambda e: self._move_cursor(1, 0),
            "select_confirm": lambda e: self._activate_current(),
        }
        for action, handler in bindings.items():
            seq = self.keybindings.sequence(action)
            self.canvas.bind(seq, handler)
            self._bound_seqs[action] = seq
    ...
    def _select_index(self, index):
        """Highlight `index` AND immediately open it (used for mouse clicks,
        where "pick" and "confirm" are naturally the same action)."""
        if not self._highlight_index(index):
            return
        self._activate_current()

    def _highlight_index(self, index):
        """Move the highlighted cursor to `index` without opening it. Used
        by keyboard arrow navigation, so browsing the gallery doesn't jump
        straight to the Pixelate tab on every key press. Returns True if
        the index was valid and applied."""
        ...
        self.selection_label.config(text=f"Highlighted: {self.titles[index]}")
        return True

    def _move_cursor(self, dx, dy):
        """Arrow-key navigation: step the highlighted cursor one cell in
        the gallery grid, clamped to stay in bounds. Row-aware (not just a
        linear +-1 like Shift+wheel), since the gallery lays out in columns."""
        if not self.titles or self.columns <= 0:
            return "break"
        ...
        row, col = divmod(self.current_index, self.columns)
        row = max(0, row + dy)
        col = min(max(0, col + dx), self.columns - 1)
        new_index = row * self.columns + col
        new_index = min(max(new_index, 0), len(self.titles) - 1)
        self._highlight_index(new_index)
        return "break"

    def _activate_current(self):
        """Confirm the currently highlighted image (Enter key, or a direct
        mouse click via _select_index) - actually opens it in the Pixelate tab."""
        if self.current_index < 0 or self.current_index >= len(self.titles):
            return "break"
