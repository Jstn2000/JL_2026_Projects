...
class PaintTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        self.selected_indicator_label.pack(pady=(0, 12))
        ...
        ttk.Label(side, text="Middle-click: reset zoom", foreground=FG_MUTED).pack(anchor="w", pady=(2, 0))
        ttk.Label(side, text="Scroll wheel: pan vertically", foreground=FG_MUTED).pack(anchor="w", pady=(2, 0))

        ttk.Button(side, text="Clear painting", command=self._clear_painting).pack(fill=tk.X, pady=(8, 0))

        ttk.Separator(side, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=6)

        # 3. Overall progress counter (per-color remaining counts now live
        # directly under each palette swatch instead of a single line here).
    ...
    def _get_unpainted_tile(self, target_color, highlighted):
        """Pre-rendered zoom x zoom tile for an unpainted cell (background +
        number + grid border). Built once per (color, highlighted) pair and
        reused - this is what makes highlighting a whole color's worth of
        cells fast: no per-cell font rendering, just a cheap paste()."""
        key = ("u", target_color, highlighted)
        tile = self._tile_cache.get(key)
        if tile is None:
            size = self._izoom
            bg = HIGHLIGHT if highlighted else WHITE
    ...
    def _draw_cell(self, gx, gy):
        ...
        if (gx, gy) in self.painted:
            ...
            tile = self._get_unpainted_tile(target_color, target_color == self.selected_color)
    ...
    def _on_mouse_move(self, event):
        ...
        if 0 <= gx < self.grid_w and 0 <= gy < self.grid_h:
            target_color = self.target_pixels[gx, gy]
            number = self.color_to_number.get(target_color, "?")
            state = "painted" if (gx, gy) in self.painted else "unpainted"
